...
Object.entries(kasutusviisidByTool).forEach(([key, usages]) => {
    if (tools[key]) tools[key].kasutusviisid = usages;
});
...
// Load tool details from URL parameters or default
function loadToolDetails() {
    const urlParams = new URLSearchParams(window.location.search);
    const toolId = urlParams.get('tool') || 'monica';

    console.log('Loading tool details for ID:', toolId); // Debug
    console.log('Available tools:', Object.keys(toolsData)); // Debug

    const toolData = toolsData[toolId];

    if (!toolData) {
        // If tool not found, show error message instead of redirecting
        console.log('Tool not found:', toolId);
        document.getElementById('detail-title').textContent = 'Tööriist ei leitud';
        document.getElementById('detail-description').innerHTML = '<p>Kahjuks selle tööriista kohta pole veel detailset infot saadaval.</p>';
        return;
    }
    ...
    toolData.badges.forEach(badge => {
        const badgeEl = document.createElement('span');
        badgeEl.className = 'hot-badge';
    ...
    // Update website link
    document.getElementById('tool-website').href = toolData.website;

    // Update description
    document.getElementById('detail-description').innerHTML = `<p>${toolData.description}</p>`;
    ...
    toolData.pricing.forEach(plan => {
        const planEl = document.createElement('div');
        planEl.className = `pricing-plan ${plan.featured ? 'featured' : ''}`;
        ...
        planEl.innerHTML = `
            ...
            <ul>${featuresHtml}</ul>
        `;
        pricingContainer.appendChild(planEl);
    });

    // Update use cases
    const usecasesContainer = document.getElementById('detail-usecases');
    usecasesContainer.innerHTML = '';
    const useCases = toolData.kasutusviisid || toolData.useCases || [];
    useCases.forEach(usecase => {
        const usecaseEl = document.createElement('div');
        usecaseEl.className = 'usecase-item';
        usecaseEl.innerHTML = `
            <h3>${usecase.title}</h3>
            <p>${usecase.description}</p>
        `;
        usecasesContainer.appendChild(usecaseEl);
    });
}
...
function loadRelatedTools() {
    const urlParams = new URLSearchParams(window.location.search);
    const currentToolId = urlParams.get('tool') || 'monica';
    const currentTool = toolsData[currentToolId];

    if (!currentTool) return;

    // Find tools in the same category (excluding current tool)
    const relatedTools = Object.entries(toolsData)
        .filter(([id, tool]) => id !== currentToolId && tool.category === currentTool.category)
        .slice(0, 3); // Show max 3 related tools

    const relatedContainer = document.getElementById('related-tools-grid');
    relatedContainer.innerHTML = '';

    relatedTools.forEach(([id, tool]) => {
        const toolCard = document.createElement('div');
        toolCard.className = 'tool-card';
        toolCard.style.cursor = 'pointer';

        const badgesHtml = tool.badges.map(badge =>
            `<span class="hot-badge">${badge}</span>`
        ).join('');
        ...
        toolCard.addEventListener('click', function () {
            window.location.href = `tool-detail.html?tool=${id}`;
        });

        relatedContainer.appendChild(toolCard);
    });

    // Hide related tools section if no related tools
    if (relatedTools.length === 0) {
        document.querySelector('.related-tools').style.display = 'none';
    }
}
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    likeBtn.addEventListener('click', function () {
        // Toggle like state
        const isLiked = this.style.background === 'rgb(255, 107, 107)';
        if (isLiked) {
            this.style.background = '#f8f9fa';
            this.style.color = '#495057';
            this.innerHTML = '<span>🤍</span> Meeldib';
        } else {
            this.style.background = '#ff6b6b';
            this.style.color = 'white';
            this.innerHTML = '<span>❤️</span> Meeldib';
        }
    });

    // Promote button functionality
    const promoteBtn = document.querySelector('.btn-promote');
    promoteBtn.addEventListener('click', function () {
        alert('Reklaamimise funktsioon tuleb peagi!');
    });
});

// Laienda toolsData objekti
Object.assign(toolsData, tools);
